...
_ICON_PATHS = {
    ...
    "warehouse": '<path d="M3 21V10l9-6 9 6v11"/><path d="M3 21h18"/><path d="M9 21v-7h6v7"/><path d="M7 10.5V21M17 10.5V21"/>',
}
...
def ask(question, history=None, scope="All"):
    ...
    # Long spinner text only on this session's FIRST message, since only the
    # first Ollama call after startup pays the model-load-into-RAM cost
    # (60-120s on CPU) - every call after that is warm (keep_alive="30m" in
    # _ask_ollama) and genuinely fast, so repeating the "can take a minute"
    # caveat on every message is misleading, not just repetitive.
    first_call = not st.session_state.get("llm_warmed", False)
    spinner_text = ("Thinking... (first Ollama call after startup can take a minute)"
                     if first_call else "Thinking...")
...
def show_item_dialog(item_number):
    ...
    vol = it.get("bottle_volume_ml")
    pack = it.get("pack")
    if vol or pack:
        size_bits = []
        if vol: size_bits.append(f"{vol} mL bottle")
        if pack: size_bits.append(f"case of {pack}")
        st.caption(" &middot; ".join(size_bits))
...
NAV = [("Dashboard", "dashboard"), ("Chat Assistant", "chat"), ("Shipments", "truck"),
       ("Inventory", "box"), ("Warehouse", "warehouse"), ("Risk Monitoring", "shield"),
       ("CRM / Customers", "users"), ("Reports", "report"), ("Alerts", "bell"), ("Settings", "settings")]
...
warehouse = data.get("warehouse", {"tracked": 0, "at_risk": 0, "flags": [], "items": []})
...
def render_warehouse():
    page_head("Warehouse", "Ankeny state warehouse - aggregate stock across all tracked stores")
    if warehouse.get("tracked", 0) == 0:
        st.warning("No warehouse data found. Run `python add_warehouse_table.py` then "
                   "`python warehouse_simulate.py`, then refresh the pipeline.")
        return

    def _with_size(rows):
        """Append bottle size to the display name so same-named items at
        different volumes (e.g. two 'Black Velvet' rows) are distinguishable
        in every table/chart below - same fix as the Reports tab."""
        out = []
        for r in rows:
            r = dict(r)
            vol = r.get("bottle_volume_ml")
            r["display_name"] = f"{r['item_name']} ({vol}mL)" if vol else r["item_name"]
            out.append(r)
        return out

    items = _with_size(warehouse.get("items", []))
    flags = _with_size(warehouse.get("flags", []))
    total_on_hand = sum(i["on_hand"] for i in items)
    m1, m2, m3, m4 = st.columns(4)
    with m1: kpi_card("warehouse", "#5b8cff", "Items tracked", warehouse["tracked"], "at Ankeny")
    with m2: kpi_card("alert", "#f56565", "At/below reorder point", warehouse["at_risk"], "need a factory order")
    with m3: kpi_card("box", "#37d399", "Total bottles on hand", f"{total_on_hand:,}", "across all items")
    with m4:
        pct = round((warehouse["at_risk"] / warehouse["tracked"]) * 100) if warehouse["tracked"] else 0
        kpi_card("trenddown", "#f5b544", "Share flagged", f"{pct}%", "of tracked items")
    st.caption("Warehouse reorder_point uses the SAME (s, S) safety-stock formula as store-level "
               "inventory, but with demand AGGREGATED across every tracked store, and vendor "
               "(factory) lead time instead of store lead time. Warehouse on_hand is a documented "
               "starting assumption (seeded full) - see warehouse_simulate.py.")
    st.write("")

    left, right = st.columns([1.6, 1])
    with left:
        st.markdown('<div class="pc-card">', unsafe_allow_html=True)
        sect("Items at/below reorder point", "alert")
        if flags:
            df = pd.DataFrame(flags)
            shown = df[["display_name", "on_hand", "reorder_point", "min_count", "max_count", "last_delivery_date"]]
            st.dataframe(shown, width='stretch', hide_index=True, column_config={
                "display_name": st.column_config.TextColumn("Item"),
                "on_hand": st.column_config.NumberColumn("On hand"),
                "reorder_point": st.column_config.NumberColumn("Reorder at"),
                "min_count": st.column_config.NumberColumn("Safety stock"),
                "max_count": st.column_config.NumberColumn("Order-up-to"),
                "last_delivery_date": st.column_config.TextColumn("Last delivery")})
            st.download_button("Export CSV", shown.to_csv(index=False), "warehouse_flags.csv", "text/csv")
        else:
            st.success("No items currently at or below their warehouse reorder point.")
        st.markdown('</div>', unsafe_allow_html=True)
    with right:
        st.markdown('<div class="pc-card">', unsafe_allow_html=True)
        sect("Stock vs reorder point", "activity")
        if items:
            df = pd.DataFrame(items)
            df["share_of_max"] = (df["on_hand"] / df["max_count"]).clip(upper=1.0)
            ch = alt.Chart(df).mark_bar(color="#5b8cff", cornerRadius=3).encode(
                x=alt.X("share_of_max:Q", title="On hand / order-up-to level",
                        axis=alt.Axis(format="%")),
                y=alt.Y("display_name:N", sort="-x", title=None)
            ).properties(height=max(230, 22 * len(df)), width="container")
            st.altair_chart(ch)
        st.markdown('</div>', unsafe_allow_html=True)

    st.write("")
    st.markdown('<div class="pc-card">', unsafe_allow_html=True)
    sect("Full warehouse catalogue", "box")
    if items:
        df = pd.DataFrame(items)
        st.dataframe(df[["display_name", "on_hand", "min_count", "reorder_point", "max_count",
                         "last_delivery_date"]], width='stretch', hide_index=True, column_config={
            "display_name": st.column_config.TextColumn("Item"),
            "on_hand": st.column_config.NumberColumn("On hand"),
            "min_count": st.column_config.NumberColumn("Safety stock"),
            "reorder_point": st.column_config.NumberColumn("Reorder at"),
    ...
    for i in range(0, len(rows), ncols):
        ...
        for c, item in zip(cols, rows[i:i + ncols]):
            with c:
                ...
                vol = item.get("bottle_volume_ml")
                pack = item.get("pack")
                size_line = (f'{vol} mL' if vol else '') + (f' &middot; case of {pack}' if pack else '')
                st.markdown(f"""<div class="pc-card" style="min-height:158px;margin-bottom:12px">
                    ...
                    <div style="color:var(--faint);font-size:.7rem;margin-top:2px">{size_line}</div>
...
def render_page():
    ...
    elif page == "Warehouse": render_warehouse()
